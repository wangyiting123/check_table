...
import  { TableList, TabList, showDialog, FormLabel } from '../../components';
import { connect } from 'react-redux';
import { initTabList, addTab, deleteTab, renameTab } from '../../store/temperatureTab/action';
  ...
  tabListChanged = (data: any) => {
    if (data.type === 'add') {
      ...
      showDialog({
        ...
        title: '建表',
        confirm: () => {
          const newTab = { id: this.props.tabList.length + 1, name: this.newTabData, type: CONSTANTS.TAB_SELF_TYPE, key: this.selectTabData};
...
tabName =(data: ITabItem, index: string) => {
  this.dropDownList = [{id: 1, name: '删除表', click: this.deleteTab, parentParams: {...data, index} },
  ...
  getTabList = () => {
    return this.props.tabList.map((item: any, index: string) => {
  ...
  render() {
    ...
    return  <TabList list={tabData} onChange={this.tabListChanged}></TabList>
  }
}
